...
- **agent** – FastAPI service (`src/agent/`) exposing `/healthz`, `/v1/agent`, and an OpenAI-compatible `/v1/chat/completions` endpoint that orchestrates memory, tools, and LiteLLM calls while returning structured responses (`steps`, `response`, `metadata`).
